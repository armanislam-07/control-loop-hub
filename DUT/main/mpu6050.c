...
static esp_err_t mpu6050_write(mpu6050_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *const data_buf, const uint8_t data_len)
{
    ...
    esp_err_t ret = i2c_master_transmit(sens->handle, write_buf, sizeof(write_buf), 1000);
...
{
    mpu6050_dev_t *sens = (mpu6050_dev_t *) sensor;
    esp_err_t  ret = i2c_master_transmit_receive(sens->handle, &reg_start_addr, 1, data_buf, data_len, 1000);
...
//don't inlucde where the ports are inside the .c files
    ...
    i2c_master_bus_handle_t bus_handle; // bus should be initalized externally? check on this (its because there is no way to clean up the bus handle later if created internally
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(bus_handle, &cfg, &dev_handle));
    sensor->handle = dev_handle;
    sensor->bus = port;
...
{
    mpu6050_dev_t *sens = (mpu6050_dev_t *) sensor;
    i2c_master_bus_rm_device(sens->handle);
    free(sens);
}
